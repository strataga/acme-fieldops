...
- After the seed, every change uses a signed pull request. Run `bun run check`, which currently delegates to `bun scripts/check-repository.ts`; code phases require at least 80% coverage.
